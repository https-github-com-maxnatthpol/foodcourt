...
for ($i=0; $i < count($_POST["Chk"]); $i++) { 
	...
	$str = "UPDATE `features` SET `delete_datetime`= '".$date_regdate."' WHERE `id_feature`= '".$id."' ";
	$objQuery = $db->Query($str);
  ...
  if (!in_array(strtolower($mimeType), $allowedMimeTypes) || !in_array(strtolower($extension), $allowedExts)) {
    ...
    $newname_img = "";
				...
				if (isset($_POST["image_ed"]) && $_POST["image_ed"] !='') {
				  $strSQL = "SELECT `id_feature`, `name_th`, `name_en`, `description_th`, `description_en`, `order`, `image`, `directory` FROM `features` WHERE id_feature = '".$_POST["id_edit"]."' ";
				  $objQuery = $db->Query($strSQL);
				  $objResult = mysqli_fetch_array($objQuery, MYSQLI_ASSOC);
					unlink($objResult["directory"].$_POST["image_ed"]);
				}
	...
	$str = "UPDATE `features` SET `name_th`='".$name_th."',`name_en`='".$name_en."',`description_th`='".$detail_th."',`description_en`='".$detail_en."',`order`='".$order."' ,`update_id`='".$id_data."',`update_datetime`='".$date_regdate."' ".$sql_img." WHERE `id_feature`= '".$id_edit."'";
	$objQuery = $db->Query($str);
	if($objQuery){
		echo json_encode(array('status' => '0','message'=> 'สำเร็จ'));
	}else{
