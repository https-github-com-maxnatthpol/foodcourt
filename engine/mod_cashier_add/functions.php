...
if (isset($_SESSION["id_data"])) {
	...
	$sql = '';
	$sql .= "SELECT COUNT(`id_cashier`) AS num_id FROM `mod_cashier` WHERE `email` = '".$e_mail."'  ";
	if ($_POST["id_cus"]!="") {
		$sql .= " AND id_cashier !='".$_POST["id_cus"]."' ";
	}
...
if (isset($_SESSION["id_data"])) {
	...
	if ($_POST["id_address"] == '') {
		$str = "INSERT INTO `user_address`( `id_user`, `address`, `district`, `amphur`, `province`, `postcode`, `type`, `status`, `create_id`, `create_datetime`) VALUES ('".$id_edit."','".$address."','".$district."','".$amphures."','".$province."','".$zip_code."','1','2','".$id_data."','".$date_regdate."') ";
...
for ($i=0; $i < count($_POST["Chk"]); $i++) { 
	...
	$str = "UPDATE `mod_cashier` SET `status`='1' WHERE `id_cashier`='".$id."' ";
	$objQuery = $db->Query($str);
...
for ($i=0; $i < count($_POST["Chk"]); $i++) { 
	...
	$str = "UPDATE `mod_cashier` SET `status`='3' WHERE `id_cashier`='".$id."' ";
	$objQuery = $db->Query($str);
...
for ($i=0; $i < count($_POST["Chk"]); $i++) { 
	...
	$str = "UPDATE `mod_cashier` SET `delete_datetime`='".$date_regdate."' WHERE `id_cashier`='".$id."' ";
	$objQuery = $db->Query($str);
...
if( file_exists("../../uploads/".$year) ){
	if( file_exists("../../uploads/".$year."/mod_cashier") ){
	}else{ 
		mkdir("../../uploads/".$year."/mod_cashier");
	}
...
$directory = "../../uploads/".$year."/mod_cashier/";	
...
if(isset($_FILES['name_img']) && $_FILES['name_img']['name']!=''){
  ...
  if (!in_array(strtolower($mimeType), $allowedMimeTypes) || !in_array(strtolower($extension), $allowedExts)) {
    //type ไม่ผ่าน
				...
				$name = "cashier-".(Date("dmy").rand('1000','9999').$sur); //ผมตั้งเป็น วันที่_เวลา.นามสกุล
...
$id_cashier = setMD5();
$str = "INSERT INTO `mod_cashier`(`id_cashier`, `forename`, `surename`, `id_card`, `email`, `telephone`, `create_id`, `create_datetime`) VALUES ('".$id_cashier."','".$name."','".$surname."','".$id_cade."','".$e_mail."','".$telaphone."','".$id_data."','".$date_regdate."')";
$objQuery = $db->Query($str);
...
$sql = "INSERT INTO `user_images`(`id_user`, `name`, `size`, `date`, `directory`, `active`, `type`) VALUES ('".$id_cashier."','".$newname_img."','".$tmp_size."','".$date_regdate."','".$directory."','1','2')";
$query = $db->Query($sql);
