fetch_data_table_list_history();
function fetch_data_table_list_history() {
  ...
  $.ajax({
    ...
    method: "POST",
    data: {
      form: "select_table_list_history",
    ...
    success: function (data) {
      $("#div_table_list_history").html(data);
      $("#table_front_manage_history").DataTable({
        scrollY: true,
        scrollCollapse: true,
        scrollX: true,
        searching: true,
...
$(document).on("click", "#btnbill_check", function () {
  var money = $("#money").val();
  var total2 = $("#total2").val();

  if (money == "" || total2 == "") {
    if (money == "") {
      $("#money").attr("style", "border-color: red; border-width: 1px;");
    } else {
      $("#money").attr("style", "");
    }

    if (total2 == "") {
      $("#total2").attr("style", "border-color: red; border-width: 1px;");
    } else {
      $("#total2").attr("style", "");
    }
  ...
  var formData = new FormData($("#form_bill_check")[0]);
  swal
    .fire({
      title: "ยืนยัน?",
      text: "ยืนยันการเช็คบิลหรือไม่?",
      icon: "info",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
    ...
    .then((result) => {
      if (result.value) {
        $.ajax({
          ...
          success: function (data) {
            if (data.status == "0") {
              swal.fire("สำเร็จ", "เช็คบิลเรียบร้อยแล้ว", "success");
              fetch_data_table();
              fetch_data_table_list_history();
              document.getElementById("form_bill_check").reset();
              $("#modal_bill_check").modal("toggle");
              ...
              let link_url =
                ...
                pieces[5] +
                "/";
              window.open(link_url+'print_bill_check.php?bill_id='+data.id_bill, '_blank', 'location=yes,height=450,width=400,scrollbars=yes,status=yes');
            } else {
              swal.fire("ไม่สำเร็จ", "เกิดปัญหากับระบบ", "warning");
            }
          },
