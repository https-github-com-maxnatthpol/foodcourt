  ...
  if (id_courese != "") {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        console.log(quiz_name_th_taxteditor);
        ...
        if (data.suggest_flg == "1") {
          document.getElementById("div_suggest_flg").innerHTML =
            '<input checked disabled name="suggest_flg" id="suggest_flg" value="1" type="checkbox"  data-off-color="danger" data-on-color="success" data-off-text="<i class=' +
            "'mdi mdi-close'" +
        ...
        if (data.popular_flg == "1") {
          document.getElementById("div_popular_flg").innerHTML =
            '<input checked disabled name="popular_flg" id="popular_flg" value="1" type="checkbox"  data-off-color="danger" data-on-color="success" checked data-off-text="<i class=' +
            "'mdi mdi-close'" +
            '></i>" data-on-text="<i class=' +
            "'mdi mdi-check'" +
            '></i>"> ';
        ...
        document.getElementById("div_img_ed").innerHTML =
          '<input disabled type="hidden" name="directory_ed" value="' +
          data.directory +
  ...
  swal
    ...
    .then(result => {
      if (result.value) {
        $.ajax({
          method: "POST",
          url: "functions.php?id_lesson=" + id_lesson,
          data: formData,
          cache: false,
          contentType: false,
          processData: false,
...
$(document).on("click", ".edit_btn_content", function() {
  ...
  if (name_img != "") {
    ...
    document.getElementById("div_img_content").innerHTML =
      "<input " +
      directory_name_img +
      ' disabled accept="image/*,application/pdf,application/x-pdf,application/msword,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation,application/pdf,video/*" type="file" id="file_content" class="dropify" name="file_content" />';
    $(".dropify").dropify({
      messages: {
        default: "Drag and drop a file here or click",
        replace: "Drag and drop or click to replace",
...
function ClickCheckAll_content(vol) {
  var i = 1;
  for (i = 1; i <= document.frmMain_content.hdnCount_content.value; i++) {
    $(".num_content_").html("[ " + i + " ]");
    if (vol.checked == true) {
      eval("document.frmMain_content.Chk_content" + i + ".checked=true");
      $(".show-tr_content").addClass("remove-item");
      $("#MultiDelete_content").prop("disabled", false);
    } else {
      $(".num_content_").html("");
      eval("document.frmMain_content.Chk_content" + i + ".checked=false");
      $("#MultiDelete_content").prop("disabled", true);
...
function edit_pic(data, directory) {
  ...
  document.getElementById("div_input_file_img").innerHTML =
    '<input  accept="image/*" ' +
    directory_name_img +
    ' type="file" disabled id="name_img_quiz_question" class="dropify" name="name_img_quiz_question"  />';
  $(".dropify").dropify({
    messages: {
      default: "Drag and drop a file here or click",
      replace: "Drag and drop or click to replace",
