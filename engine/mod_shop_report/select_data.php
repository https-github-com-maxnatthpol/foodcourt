      ...
      <tr class="show-tr">
        ...
        <td>
          <?php if ($row_sales_store == 0) { ?>
              <button type="button" class="btn btn-warning btn-sm approval_btn_product" style="<?php echo $button_approval ?>" data-id="<?php echo $objResult['id_customer'] ?>" data-amount-customer="<?php echo ($objResult_amount['amount_customer']) ?>" data-amount-percent="<?php echo ($objResult_amount_percent['amount_percent']) ?>" data-total_cus_per="<?php echo $total_cus_per; ?>" data-date_action="<?php echo $customer_date; ?>" ><i class="fas fa-question-circle"></i>&nbsp;อนุมัติการจ่ายเงิน</button>
          <?php } else { ?>
