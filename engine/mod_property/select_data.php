...
$button_view   = $_POST["button_view"];
  ...
  $strSQL = "SELECT `id_property`,`name_th`,`order`,`image`,`directory` FROM `property` WHERE `delete_datetime` IS null ORDER BY `order` ASC";
...
?>
      <tr class="show-tr">
        <td>
          <input type="checkbox" name="Chk[]" id="Chk<?php echo $num ?>" value="<?php echo $objResult['id_property'] ?>" class="checkbox_remove filled-in chk-col-light-blue" /><label for="Chk<?php echo $num ?>"></label>
...
if ($objResult["image"]=='') {
  ...
  $img = $objResult["directory"].$objResult["image"];
        ...
        <td>
          ...
          <a data-toggle="modal" data-target="#modal_edit" style="<?php echo $button_update ?>"  class="btn btn-warning  btn-sm edit_btn" data-id="<?php echo $objResult['id_property'] ?>" ><i class="fa fa-edit" style="color: white;"></i></a>

          <button type="button" class="btn btn-danger  btn-sm delete_btn" style="<?php echo $button_delete ?>" data-id="<?php echo $objResult['id_property'] ?>"><i class="mdi mdi-delete-empty" style="color: white;"></i></button>
        </td>
      </tr>
<?php
...
function select_div_edit_front_manage(){
  ...
  $strSQL = "SELECT `id_property`, `name_th`, `name_en`, `description_th`, `description_en`, `order`, `image`, `directory` FROM `property` WHERE id_property = '".$_POST["id"]."' ";
  $objQuery = $db->Query($strSQL);
                                ...
                                <div class="tab-pane active" id="thai_edit" role="tabpanel">
                                    <div class="card-body">
                                      ...
                                      <div class="form-group">
                                        ...
                                        <input onkeyup ="disabled_btn_edit_property()" type="text" class="form-control" id="name_th_edit" name="name_th" placeholder="ภาษาไทย" value="<?php echo $objResult["name_th"] ?>">
                                      </div>
...
?>
<input type="hidden" name="image_ed" value="<?php echo $image_ed ?>">  
                    <div class="form-group col-md-12">
                      <div class="card">
                        <div class="card-body ">
                          <label for="input-file-now">รูปภาพ</label>
                          <input onchange="chk_pic_edit()" accept="image/*" type="file" id="property_img_edit" class="dropify" name="property_img" <?php echo $image ?> />
                        </div>
                      </div>
                    </div>
              </div>
