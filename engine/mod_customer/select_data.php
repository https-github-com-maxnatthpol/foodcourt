...
function select_div_edit_front_manage(){
	...
	$strSQL = "SELECT  user_images.name AS name_img, user_images.directory, mod_customer.forename, mod_customer.surename, mod_customer.id_card, mod_customer.license_number, mod_customer.email,mod_customer.telephone
FROM `mod_customer` 
...
?>
                    ...
                    <div class="form-group">
                      <label for="example-email">เลขที่ใบอนุญาตประกอบวิชาชีพการสัตวแพทย์ </label>
                      <input  type="text" class="form-control" id="license_number_edit" name="license_number" pattern="[0-9A-Za-z]{2}-[0-9A-Za-z]{4}/[0-9A-Za-z]{4}"   placeholder="xx-xxxx/xxxx" value="<?php echo $objResult["license_number"] ?>" data-mask="**-****/****" required>
                    </div>
...
$button_view   = $_POST["button_view"];
	...
	$strSQL = "SELECT `id_customer`,`forename`,`surename`,`license_number`,`email`,`status` FROM `mod_customer` WHERE `delete_datetime` IS null  ORDER BY `create_datetime` DESC ";
	$objQuery = $db->Query($strSQL);
...
?>
        ...
        <td>
          <?php echo $objResult["license_number"] ?>
        </td>
        <td>
        ...
        <td>
          ...
          ?>
				...
				<td>
					<a data-toggle="modal" data-target="#modal_edit" style="<?php echo $button_update ?>"  class="btn btn-warning  btn-sm edit_btn" data-id="<?php echo $objResult['id_customer'] ?>"  ><i class="fa fa-edit"></i></a>
          ...
          <a data-toggle="modal" data-target="#modal_address" style="<?php echo $button_update ?>"  class="btn btn-outline-secondary  btn-sm address_btn" data-id="<?php echo $objResult['id_customer'] ?>"  data1-id="<?php echo $objResult["forename"].' '.$objResult["surename"] ?>" >ที่อยู่</a>
