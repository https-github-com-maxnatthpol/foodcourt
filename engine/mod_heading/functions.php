...
if(isset($_FILES['heading_img']) && $_FILES['heading_img']['name']!=''){
  ...
  if (!in_array(strtolower($mimeType), $allowedMimeTypes) || !in_array(strtolower($extension), $allowedExts)) {
    ...
    $newname_img = "";
				...
				$name = "heading-".(Date("dmy").rand('1000','9999').$sur); //ผมตั้งเป็น วันที่_เวลา.นามสกุล
...
if( file_exists("../../uploads/".$year) ){
	if( file_exists("../../uploads/".$year."/mod_fuature") ){
	}else{ 
		mkdir("../../uploads/".$year."/mod_fuature");
	}
}else{ 
	mkdir("../../uploads/".$year);
	if( file_exists("../../uploads/".$year."/mod_fuature") ){
...
$directory = "../../uploads/".$year."/mod_fuature/";

if(isset($_FILES['fuature_img']) && $_FILES['fuature_img']['name']!=''){

  $fieldname = $_FILES['fuature_img']['name'];
  // Get filename.
  $filename = explode(".", $_FILES['fuature_img']['name']);
  $finfo = finfo_open(FILEINFO_MIME_TYPE);
  $tmpName = $_FILES['fuature_img']['tmp_name'];
  ...
  if (!in_array(strtolower($mimeType), $allowedMimeTypes) || !in_array(strtolower($extension), $allowedExts)) {
    ...
    $newname_img = "";
				...
				$namefile = $_FILES["fuature_img"]["name"];
				$sur = strrchr($namefile, "."); //ตัดนามสกุลไฟล์เก็บไว้
				$name = "fuature-".(Date("dmy").rand('1000','9999').$sur); //ผมตั้งเป็น วันที่_เวลา.นามสกุล
				$target = $directory.$name;
				$newname = $name;
				...
				copy($_FILES["fuature_img"]["tmp_name"],iconv('UTF-8','windows-874',$directory.$newname));
				$newname_img = $newname;
	}	
}else{
  	$newname_img = "";
	...
	$str = "INSERT INTO `features`(`name_th`, `name_en`, `description_th`, `description_en`, `order`, `image`, `directory`, `create_id`, `create_datetime`) VALUES ('".$name_th."','".$name_en."','".$detail_th."','".$detail_en."','".$order."','".$newname_img."','".$directory."','".$id_data."','".$date_regdate."')";
	$objQuery = $db->Query($str);
	if($objQuery){
		echo json_encode(array('status' => '0','message'=> 'สำเร็จ'));
	}else{
...
for ($i=0; $i < count($_POST["Chk"]); $i++) { 
	...
	$str = "UPDATE `heading` SET `delete_datetime`= '".$date_regdate."' WHERE `id_heading`= '".$id."' ";
	$objQuery = $db->Query($str);
  ...
  if (!in_array(strtolower($mimeType), $allowedMimeTypes) || !in_array(strtolower($extension), $allowedExts)) {
    ...
    $newname_img = "";
				...
				if (isset($_POST["image_ed"]) && $_POST["image_ed"] !='') {
				  $strSQL = "SELECT `id_heading`, `image`, `directory` FROM `heading` WHERE id_heading = '".$_POST["id_edit"]."' ";
	...
	$str = "UPDATE `heading` SET `tag`='".$tag_th_heading."',`name_th`='".$name_th."',`name_en`='".$name_en."',`description_th`='".$detail_th."',`description_en`='".$detail_en."',`update_id`='".$id_data."',`update_datetime`='".$date_regdate."' ".$sql_img." WHERE `id_heading`='".$id_edit."'";
	$objQuery = $db->Query($str);
	if($objQuery){
		echo json_encode(array('status' => '0','message'=> 'สำเร็จ'.$strSQL));
	}else{
		echo json_encode(array('status' => '1','message'=> 'ไม่สำเร็จ'));
	}
