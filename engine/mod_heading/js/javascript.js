...
$(document).on('click', '#btnSendAdd_Heading', function(){
       var formData = new FormData($('#form_add_Heading')[0]);
         swal.fire({
        title: "ยืนยัน?",
        text: "ยืนยันการบันทึกหรือไม่?",
        icon: 'info',
        ...
        $.ajax({
          ...
          success: function(data) {
            if (data.status=='0') {
            swal.fire("สำเร็จ", "บันทึกเรียบร้อยแล้ว", "success");
             document.getElementById("form_add_Heading").reset();
            var tagButton = document.getElementsByClassName("dropify-clear")[0];
//            tagButton.click();
            disabled_btn_add_heading();
            fetch_data_table();
        }else{
          swal.fire("ไม่สำเร็จ", "เกิดปัญหากับระบบ", "warning");
        }
...
function disabled_btn_add_fuature(){
...
function disabled_btn_add_heading(){
	var name_th = $('#name_th_heading').val();
	if (name_th == '') {
		document.getElementById('btnSendAdd_Heading').disabled = true;
	}else{
		document.getElementById('btnSendAdd_Heading').disabled = false;
	}
}
...
function fetch_data_table() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#div_table_front_manage').html(data);
...
//            tagButton.click();
   ...
   function disabled_btn_edit_heading(){
  var name_th = $('#name_th_heading_edit').val();
  if (name_th == '') {
